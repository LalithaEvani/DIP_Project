...
def crop_and_save_lines(text_image, lines_binary_image, output_folder,image_name):
    ...
    for i in range(1, num_labels):  # Skip the background label (0)
        ...
        if w> lines_binary_image.shape[1]/2.5:
            stats_n.append(stats[i])

    # print('number of lines:',len(stats_n))
    ...
    for i in range(len(stats_n)):
        ...
        line_image_path = os.path.join(output_folder, f'{image_name}_{i}.png')
        # print(f'image_name {image_name}')
        # print(f'output folder {output_folder}')
        # print(f'line_image path {line_image_path}')

        cropped_line_n = np.transpose(cropped_line_n, (1, 0))  # Transpose the rows and columns
        cropped_line_n = np.flip(cropped_line_n, axis=0)  # Flip along the vertical axis
        # f=cv2.imwrite(line_image_path, cropped_line_n)

        f=cv2.imwrite(line_image_path,cv2.bitwise_not(cropped_line_n) )

        # plt.imshow(cropped_line_n)
        # plt.show()
...
def PPA_Algo_word(line_image_path, output_folder, image_line_name, vartical_gap_threshold = 10):
    image = cv2.imread(line_image_path, cv2.IMREAD_GRAYSCALE)

    rotated_image = np.transpose(image, (1, 0))  # Transpose the rows and columns
    rotated_image = np.flip(rotated_image, axis=1)  # Flip along the vertical axis
    image = rotated_image
    ...
    stripe_width = int(average_width/2)
...
#                 filled_image_n[j:j+threshold_width, i:i + stripe_width]=255
    ...
    gap_threshold =  filled_image.shape[1]/2.5#edit: changing to 2.5 from 1.5

#   Complementing and thinning operations 
    # filled_image_n_n = fill_horizontal_white_gaps(filled_image_n, gap_threshold)
    filled_image_n_n = fill_horizontal_white_gaps(filled_image, gap_threshold)

    skeleton = skeletoniz_n(filled_image_n_n)
    # skeleton = skeletoniz_n(filled_image)
...
#   Trimming process
    ...
    # Minimum line length (1/1.5 of image width)
    min_length = image_width / 1.5
    ...
    max_distance = image_width / 5
    if n_lines!=0:
        completed_image = connect_lines(skeleton, endpoints, n_lines, max_distance)
    else:
        completed_image=skeleton
    # Connect endpoints and extend lines horizontally
    # image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
    _, binary_image = cv2.threshold(image, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    completed_image_c = completed_image.copy()
    crop_and_save_lines(binary_image, completed_image_c, output_folder,image_line_name)

    return painted_binary_image,filled_image_n_n, thresholded_image,filtered_image,skeleton,completed_image,binary_image+255*completed_image
