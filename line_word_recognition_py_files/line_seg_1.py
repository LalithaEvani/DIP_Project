...
def crop_and_save_lines(text_image, lines_binary_image, output_folder):
    ...
    for i in range(len(stats_n)):
        ...
        if cropped_line_n.shape[0]< (text_image.shape[0]/(2*len(stats_n))):
            continue
        line_image_path = os.path.join(output_folder, f'{line_num}.png')
        line_num+=1
       
        f = cv2.imwrite(line_image_path, cv2.bitwise_not(cropped_line_n))
      
    print('number of lines detected:',line_num)
...
def find_endpoints(skeleton):
    ...
    endpoints_filtered_n = [endpoint for endpoint in endpoints_n if endpoint[0] > (skeleton.shape[0] / 4)] #edited
